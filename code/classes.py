...
class CubeAruco(System):
    ...
    def boards(self):
        return [cv2.aruco.GridBoard_create(2, 2, self.marker_size, self.marker_sep, self.aruco_dict, j) for j in
                self.marker_ids]
    ...
    def cube_pose(self, corners, ids, face):
        ...
        if corners is not None:
            ...
            r_, r_vector, t_vector = cv2.aruco.estimatePoseBoard(corners, ids, board[face], self.c_mtx, self.c_dist,
                                                                 r_vec_, t_vec_)
